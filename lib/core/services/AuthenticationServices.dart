...
class AuthenticationServices extends Services {
  ...
  Future checkDetails({
    ...
    await _schoolLoginRef.get().then((onValue) {
      ...
      print("Inside Then :" + onValue.documentID);
    });
    ...
    CollectionReference _userRef = _schoolLoginRef.collection('Student');
    print('Login type = ');
    await _userRef
        .where("email", isEqualTo: email)
            ...
            .first
            .then((querySnapshot) {
        print('test 1 email =  '+email);
        print(''+querySnapshot.documents.isEmpty.toString());
        querySnapshot.documents.forEach((documentSnapshot) {
        print('test 1');
        isUserAvailable = documentSnapshot.exists;
        String userAvailable = isUserAvailable?'yes':'no';
        print('logintype= isUserAvailable'+userAvailable);
